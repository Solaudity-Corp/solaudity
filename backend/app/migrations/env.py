...
def _get_sqlalchemy_url() -> str:
    ...
    if not db_path:
        raise RuntimeError(
            "DB_PATH is not set. Required for Alembic (option 2.B). Example: DB_PATH=/data/solaudity.db"
        )
...
# Make sure `import app.*` works when running Alembic from `backend/app/`
BACKEND_DIR = Path(__file__).resolve().parents[2]  # .../backend
if str(BACKEND_DIR) not in sys.path:
    sys.path.append(str(BACKEND_DIR))

# Option 2.B: derive URL from DB_PATH at runtime
